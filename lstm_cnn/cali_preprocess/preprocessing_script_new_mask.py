...
period_start = f'2021-{month}-01'
period_end = f'2021-{month}-28'

# set location to save preprocessed files
save_folder_location = f'/home/capstone22/WildFIrePrediction/agni/cali_data/evaluation_data/{month}/'
...
mask_feature_collection_path = 'TIGER/2016/States'
...
def drive_export_and_preprocess(image, image_id, image_date, coordinates):
    '''
    ...
    task = ee.batch.Export.image(image,
        taskname, {
            'driveFolder': f'gee_export_Data_{period_start}/{sys.argv[1]}',
            'driveFileNamePrefix': drive_filename,
            'scale':120,
        }
    )
    ...
    ## histogram to save
    convert_to_histogram(combined_data_array, file_name, target_image_data_size, coordinates)
...
def convert_to_histogram(stacked_array, file_name, image_slice_length, coordinates):
    '''
    ...
    for i in range(num_slice_length):
        for  j in range(num_slice_width):
            ...
            with open(COORD_INFO_FILE, 'a') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([slice_filename, coordinates[0], coordinates[1]])
                csvfile.close()
...
try:
    ...
    if not creds or creds.invalid:
        flow = client.flow_from_clientsecrets('/home/capstone22/WildFIrePrediction/credentials/credentials.json', SCOPES)
    ...
    california_mask = ee.FeatureCollection(mask_feature_collection_path).filter(ee.Filter.eq('NAME', 'California'))
    ...
    satellite_image_collection = ee.ImageCollection('LANDSAT/LE07/C01/T1_RT') \
        ...
        .filterBounds(california_mask.geometry()) \
    ...
    for i in tqdm(range(num_images)):
        ...
        coordinates = find_center(current_image.getInfo()['properties']['system:footprint']['coordinates'])
        ...
        while current_time > lower_bound_time:
            print(f'current_time: {current_time}; lower_bound_time: {lower_bound_time}')
        ...
        drive_export_and_preprocess(final_stacked_data_image, image_id, image_date, coordinates)
except:
    file_name = f'/home/capstone22/WildFIrePrediction/agni/preprocessing_cali/dead_processes/{period_start}_{target_i}.txt'
