...
# import tensorflow as tf # Version 2.6.2
# from tensorflow.keras.layers.CuDNNLSTM import CuDNNLSTM 
...
model_filename = 'model_cali.hdf5'
testset_log_filename = 'model_cali_test_set_log.txt'
training_dataset_fire_only_directory = '/home/capstone22/WildFIrePrediction/agni/preprocessing_cali/preprocessed_data_fire_only/'
training_dataset_directory = '/home/capstone22/WildFIrePrediction/agni/preprocessing_cali/preprocessed_data/'
testset_directory = '/home/capstone22/WildFIrePrediction/agni/preprocessing_cali/preprocessed_test_data/'
...
#     X = tf.keras.layers.Conv2D(64, kernel_size=(5, 5), strides=3, activation='relu')(X_input)
    X = tf.keras.layers.Conv2D(64, kernel_size=(3, 3), strides=1, activation='relu')(X_input)
...
def create_model():
    ...
    X_input = tf.keras.layers.Input(shape=(None, height, width, num_channel))
...
#     X = tf.keras.layers.Flatten()(X)
    # X = tf.compat.v1.keras.layers.CuDNNLSTM(64)(X)
    X = tf.keras.layers.LSTM(64)(X) # KELLY
    # X = tf.keras.layers.MultiHeadAttention(num_heads=2, key_dim=2)(X) # KELLY
...
def parse_input_data(filenames):
    ...
    #stack
    return np.stack(datas), np.concatenate(labels)
...
for i in tqdm(range(num_epoch)):
    ...
    for x in range(0, epoch_len, batch_size):
        if epoch_len - x < batch_size:
            continue

        current_files = dataset_files_list[x : x + batch_size]
        ...
        if x % 500 == 0:
            ...
            result = net.fit(x=input_datas, y=labels, epochs=1, workers=16, batch_size=batch_size)
        else:
            # start_fit = time()
            result = net.fit(x=input_datas, y=labels, epochs=1, verbose=0, workers=16, batch_size=batch_size)
            # print(f'fit time: {time() - start_fit}')
    ...
    #     # print(f'parse time: {time() - start_parse}')
    ...
    #         if not np.isnan(np.mean(loss_history)):
    #             loss_history_overall.append(np.mean(loss_history))
    #         result = net.fit(x=input_data, y=label, epochs=1, workers=16)
    #     else:
    #         start_fit = time()
    #         result = net.fit(x=input_data, y=label, epochs=1, verbose=0, workers=16)
    #         # print(f'fit time: {time() - start_fit}')
    #     loss_history.append(result.history['loss'])
